...
from triton_kernels.moe.fused_moe import fused_expert_ffn, fused_moe_forward
...
class TestMoERouterSoftmax:
    ...
    def test_softmax_weights_match_reference(self, num_tokens: int, num_experts: int, top_k: int):
        ...
        assert match_ratio >= 0.9, (
            f"Only {match_ratio:.1%} of tokens agree on expert selection (expected >= 90%)"
        )
...
class TestMoERouterEdgeCases:
    ...
    def test_no_nan_with_large_logits(self):
        ...
        hidden_states = (
            torch.randn(num_tokens, hidden_dim, device="cuda", dtype=torch.float16) * 100
        )
        router_weight = torch.randn(num_experts, hidden_dim, device="cuda", dtype=torch.float16)
    ...
    def test_no_nan_with_small_logits(self):
        ...
        num_tokens, num_experts, hidden_dim = 32, 8, 128
        hidden_states = (
            torch.randn(num_tokens, hidden_dim, device="cuda", dtype=torch.float16) * 1e-4
        )
        router_weight = (
            torch.randn(num_experts, hidden_dim, device="cuda", dtype=torch.float16) * 1e-4
        )
...
class TestMoEReferenceImplementation:
    ...
    def test_reference_permute_unpermute_roundtrip(self):
        ...
        top_k_weights = torch.softmax(
            torch.randn(num_tokens, top_k, device="cuda", dtype=torch.float32),
            dim=-1,
        ).half()
        ...
        expected = (
            hidden_states.unsqueeze(1).expand(-1, top_k, -1) * top_k_weights.unsqueeze(-1)
        ).sum(dim=1)
...
class TestPermuteTokens:
    ...
    def test_expert_contiguity(self):
        ...
        for e in range(num_experts):
            ...
            if start < end:
                assert (sorted_experts[start:end] == e).all(), f"Expert {e} tokens not contiguous"
...
class TestUnpermuteTokens:
    ...
    def test_unpermute_matches_reference(
        ...
        out_torch = unpermute_tokens_torch(
            expert_outputs, top_k_weights, restore_idx, num_tokens, top_k
        )
    ...
    def test_unpermute_output_shape(self):
        ...
        num_tokens, top_k, hidden_dim = 64, 2, 512
        expert_outputs = torch.randn(
            num_tokens * top_k, hidden_dim, device="cuda", dtype=torch.float16
        )
...
def _make_expert_data(num_tokens, num_experts, top_k, hidden_dim, ffn_dim):
    ...
    w_gate = (
        torch.randn(num_experts, ffn_dim, hidden_dim, device="cuda", dtype=torch.float16) * 0.02
    )
    w_up = torch.randn(num_experts, ffn_dim, hidden_dim, device="cuda", dtype=torch.float16) * 0.02
    w_down = (
        torch.randn(num_experts, hidden_dim, ffn_dim, device="cuda", dtype=torch.float16) * 0.02
    )
...
class TestFusedGateUp:
    ...
    def test_fused_ffn_matches_unfused(
        ...
        unfused_output = unpermute_tokens(
            unfused_expert_out, top_k_weights, restore_idx, num_tokens, top_k
        )
