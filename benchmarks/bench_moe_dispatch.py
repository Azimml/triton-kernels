...
def _create_moe_inputs(num_tokens: int, cfg: dict, device: str = "cuda"):
    ...
    w_gate = (
        torch.randn(num_experts, ffn_dim, hidden_dim, device=device, dtype=torch.float16) * 0.02
    )
    w_up = torch.randn(num_experts, ffn_dim, hidden_dim, device=device, dtype=torch.float16) * 0.02
    w_down = (
        torch.randn(num_experts, hidden_dim, ffn_dim, device=device, dtype=torch.float16) * 0.02
    )
...
def run_benchmark(
    ...
    print(
        f"Config: {cfg['num_experts']} experts, top-{cfg['top_k']}, "
...
def main():
    ...
    parser.add_argument("--all", action="store_true", help="Benchmark all model configurations")
    parser.add_argument(
        "--skip-reference", action="store_true", help="Skip PyTorch reference (faster)"
    )
