...
#
# This static table is the *fallback*: `get_gpu_specs()` first tries to derive
# the peak memory bandwidth dynamically from the CUDA device properties
# (memory clock x bus width) and only consults this table when the driver does
# not expose those fields. FP16 TFLOPS cannot be derived from device properties
# (it depends on tensor-core generation), so it always comes from the table.
GPU_SPECS = {
    # NVIDIA Data Center
...
def _bandwidth_from_device_props(props) -> float | None:
    """
    Derive theoretical peak memory bandwidth (GB/s) from CUDA device properties.

    Uses the standard formula for a GDDR/HBM bus:

        BW = 2 (DDR) * memory_clock_hz * bus_width_bytes

    `memory_clock_rate` (kHz) and `memory_bus_width` (bits) are exposed by newer
    PyTorch/CUDA builds. Returns None when either field is unavailable so the
    caller can fall back to the static table.
    """
    clock_khz = getattr(props, "memory_clock_rate", 0) or 0
    bus_bits = getattr(props, "memory_bus_width", 0) or 0
    if clock_khz <= 0 or bus_bits <= 0:
        return None
    # kHz -> Hz, bits -> bytes, x2 for double-data-rate; result in bytes/s.
    bytes_per_s = 2.0 * (clock_khz * 1e3) * (bus_bits / 8.0)
    return bytes_per_s / 1e9  # -> GB/s


def _nvidia_smi_bandwidth_gb_s() -> float | None:
    """
    Query peak memory bandwidth via `nvidia-smi` as a secondary dynamic source.

    Reads the max memory clock and bus width from nvidia-smi and applies the
    same DDR formula. Returns None if nvidia-smi is missing or the query fails
    (e.g. on a CPU-only host), so callers degrade gracefully.
    """
    ...
    if len(parts) != 2:
        return None
    try:
        clock_mhz = float(parts[0])  # MHz
        bus_bits = float(parts[1])  # bits
    except ValueError:
        return None
    if clock_mhz <= 0 or bus_bits <= 0:
        return None
    bytes_per_s = 2.0 * (clock_mhz * 1e6) * (bus_bits / 8.0)
    return bytes_per_s / 1e9


def _lookup_static_specs(name: str) -> dict:
    """Look up a GPU by name in the static table, with a partial-match fallback."""
    specs = GPU_SPECS.get(name)
    if specs is None:
        for known_name, known_specs in GPU_SPECS.items():
            if known_name != "default" and (known_name in name or name in known_name):
                specs = known_specs
                break
    if specs is None:
...
def get_gpu_specs() -> GPUSpecs:
    ...
    Memory bandwidth is derived dynamically where possible: first from the CUDA
    device properties (memory clock x bus width), then from ``nvidia-smi``, and
    finally from the static ``GPU_SPECS`` table. FP16 peak TFLOPS always comes
    from the table since it cannot be inferred from device properties.
    ...
    static_specs = _lookup_static_specs(name)

    # Prefer a dynamically derived bandwidth; fall back to the static table.
    bandwidth_gb_s = _bandwidth_from_device_props(props)
    if bandwidth_gb_s is None:
        bandwidth_gb_s = _nvidia_smi_bandwidth_gb_s()
    if bandwidth_gb_s is None:
        bandwidth_gb_s = static_specs["bandwidth_gb_s"]
    ...
    return GPUSpecs(
        ...
        fp16_tflops=static_specs["fp16_tflops"],
        num_sms=num_sms,
    )
...
class BenchmarkResult:
    ...
    @property
    def achieved_bandwidth_gb_s(self) -> float | None:
...
def plot_roofline(
    ...
    try:
        ...
        raise ImportError(
            "matplotlib and numpy required for plotting. Install with: pip install matplotlib numpy"
        ) from e
    ...
    for result in results:
        ...
        ax.scatter(
            ai, gflops, c=color, marker=marker, s=120, edgecolors="black", linewidths=0.5, zorder=3
        )
        ...
        label = f"{result.name}\n({efficiency:.0f}% eff)"
        ax.annotate(
            label, (ai, gflops), textcoords="offset points", xytext=(8, 5), fontsize=8, ha="left"
        )
    ...
    specs_text = f"Peak BW: {peak_bandwidth:.0f} GB/s\nPeak FP16: {peak_compute / 1000:.0f} TFLOPS"
    ax.text(
        0.02,
        0.98,
        specs_text,
