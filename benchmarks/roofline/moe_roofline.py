...
def profile_expert_gemm_unfused(num_tokens: int, cfg: dict) -> BenchmarkResult:
    ...
    bytes_acc = (
        total_tokens * hidden_dim * dtype_size
    )  # read permuted tokens (x3 for 3 GEMMs, but cached)
    bytes_acc += (
        num_experts * (2 * ffn_dim * hidden_dim + hidden_dim * ffn_dim) * dtype_size
    )  # weights
    bytes_acc += (
        total_tokens * ffn_dim * dtype_size * 4
    )  # gate_out + up_out write + read + intermediate write
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="mixtral-8x7b", choices=list(MODEL_CONFIGS.keys())
    )
    ...
    parser.add_argument("--all", action="store_true", help="Run for all model configs")
