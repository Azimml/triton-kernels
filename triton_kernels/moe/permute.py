...
def compute_permutation_indices(
    ...
    expert_counts.scatter_add_(
        0, flat_experts.long(), torch.ones_like(flat_experts, dtype=torch.int64)
    )
...
def permute_tokens(
    ...
    permuted = torch.empty(
        num_tokens * top_k, hidden_dim, device=hidden_states.device, dtype=hidden_states.dtype
    )
...
def unpermute_tokens(
    ...
    output = torch.empty(
        num_tokens, hidden_dim, device=expert_outputs.device, dtype=expert_outputs.dtype
    )
