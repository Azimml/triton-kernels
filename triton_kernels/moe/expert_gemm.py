...
def expert_ffn_torch(
    ...
    for e in range(num_experts):
        ...
        gate = torch.nn.functional.silu(torch.nn.functional.linear(x.float(), w_gate[e].float()))
        up = torch.nn.functional.linear(x.float(), w_up[e].float())
        intermediate = (gate * up).to(x.dtype)
        output[start:end] = torch.nn.functional.linear(intermediate.float(), w_down[e].float()).to(
            x.dtype
        )
