...
from triton_kernels.moe.expert_gemm import _build_block_schedule, grouped_gemm
...
def _fused_gate_up_kernel(
    ...
    bg_ptrs = (
        B_gate + (expert_id * N + offs_n[None, :]) * stride_bg_n + offs_k[:, None] * stride_bg_k
    )
...
def fused_expert_ffn(
    ...
    if num_blocks == 0:
        return torch.zeros(
            num_tokens, hidden_dim, device=permuted_tokens.device, dtype=permuted_tokens.dtype
        )
    ...
    intermediate = torch.empty(
        total_tokens, ffn_dim, device=permuted_tokens.device, dtype=permuted_tokens.dtype
    )
    ...
    _fused_gate_up_kernel[grid](
        ...
        w_gate_flat.stride(0),
        w_gate_flat.stride(1),
        w_up_flat.stride(0),
        w_up_flat.stride(1),
