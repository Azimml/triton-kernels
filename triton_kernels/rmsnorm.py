...
def _rmsnorm_launch_config(hidden_dim: int) -> tuple[int, int]:
    """
    Choose (BLOCK_SIZE, num_warps) for the row-wise RMSNorm kernel.

    Each program normalizes one row, so BLOCK_SIZE must cover the hidden dim
    (rounded up to a power of two, capped at 8192 to bound register pressure).
    ``num_warps`` is scaled with the block size so wide rows get more warps to
    parallelize the reduction, following the same reasoning as the Triton
    LayerNorm tutorial. This is a portable host-side heuristic (no arch probe
    needed) that replaces the previous fixed single-warp launch; it is a safe
    default across NVIDIA and AMD and requires no GPU to compute.
    """
...
def rmsnorm(
    ...
    assert x.shape[-1] == weight.shape[0], (
        f"Hidden dim mismatch: {x.shape[-1]} vs {weight.shape[0]}"
    )
    ...
    # Choose block size and warp count from the hidden dimension. Wider rows
    # get more warps so the reduction is parallelized across the program.
...
def rmsnorm_residual_fused(
    ...
    # Choose block size and warp count from the hidden dimension (see
    # _rmsnorm_launch_config).
    BLOCK_SIZE, num_warps = _rmsnorm_launch_config(hidden_dim)
