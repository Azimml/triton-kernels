...
from triton_kernels.quantization import dequantize, quantize_weight_per_channel


# Compiled dequantization kernel for best performance
@torch.compile(dynamic=True)
def _dequantize_output(
    y_int32: torch.Tensor, inv_x_scale: torch.Tensor, scale_fp16: torch.Tensor
) -> torch.Tensor:
...
def _int8_gemm_cublas(
    ...
    scale: torch.Tensor,
    scale_fp16: torch.Tensor | None = None,
...
def _int8_gemm_fused(
    ...
    weight_int8: torch.Tensor,
    scale: torch.Tensor,
    weight_transposed: torch.Tensor | None = None,
) -> torch.Tensor:
    """
...
class Int8Linear(torch.nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("scale_fp16", torch.ones(out_features, dtype=torch.float16))
        # Pre-transposed weight for Triton path
        self.register_buffer(
            "weight_transposed", torch.zeros(in_features, out_features, dtype=torch.int8)
        )
