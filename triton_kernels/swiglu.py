...
def _select_block_size(n_elements: int) -> int:
    """
    Pick an elementwise BLOCK_SIZE that keeps enough programs in flight.

    A fixed large block (e.g. 1024) launches too few programs for small
    tensors, leaving most SMs idle. This scales the block size down for small
    inputs so the grid has enough blocks to saturate the device, and caps it at
    1024 (a good balance of occupancy vs per-program register pressure) for
    large inputs. All candidates are powers of two, which the vectorized loads
    prefer. This is a pure host-side heuristic - it does not require a GPU to
    compute, so it is safe on CPU-only hosts.
    """
    if n_elements <= 4096:
        return 256
    if n_elements <= 65536:
...
def swiglu_fused(
    ...
    # Adaptive block size: small tensors use smaller blocks so the grid has
    # enough programs to fill the SMs; large tensors use 1024 for occupancy.
