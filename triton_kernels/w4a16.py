...
def _w4a16_gemm_kernel(
    ...
    stride_ak,
    stride_bk,
    stride_bn,  # strides of the PACKED weight (rows are K // 8)
    stride_sk,
    stride_sn,
    stride_zk,
    stride_zn,
...
@triton.autotune(
    configs=[
        triton.Config(
            {"BLOCK_M": 16, "BLOCK_N": bn, "BLOCK_K": 64, "SPLIT_K": sk},
            num_stages=ns,
            num_warps=nw,
        )
        for bn in (64, 128)
...
def _w4a16_gemm_splitk_kernel(
    ...
    b_ptrs = (
        B + (pid_k * (BLOCK_K // 8) + offs_kp)[:, None] * stride_bk + offs_n[None, :] * stride_bn
    )
...
def quantize_weight_int4_grouped(
    ...
    return (
        packed,
        scale.reshape(K // group_size, N).to(weight.dtype),
        zero.reshape(K // group_size, N).to(weight.dtype),
    )
