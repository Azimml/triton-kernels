...
Both kernels are `@triton.autotune`d on `key=["M", "N", "K"]`, so Triton compiles
and caches the winning config per problem shape. The candidate configs are chosen
deliberately, not swept blindly:

**Main kernel (`_w4a16_gemm_kernel`, `M > 8`).** Eight configs, all with
`BLOCK_K in {32, 64}`. `BLOCK_K` is constrained to be a multiple of 8 (so a tile
covers whole packed int32 rows) *and* a divisor of the supported group sizes
(64 / 128), which is what lets the kernel hoist a single scale/zero row per tile
instead of a per-element scale load. Small-`M` tiles (`BLOCK_M` 16/32) come first
because W4A16 is a decode/small-batch kernel; larger `BLOCK_M` (64/128) covers the
moderate-batch tail. `BLOCK_N` spans 128/256 and `num_warps` 4/8 to trade
occupancy against register pressure per shape.

**Split-K kernel (`_w4a16_gemm_splitk_kernel`, `M <= 8`).** A product sweep of
`BLOCK_N in {64, 128}`, `SPLIT_K in {4, 8, 16}`, `num_stages in {3, 4}`, and
`num_warps in {2, 4}` (24 configs), with `BLOCK_M = 16` fixed and `BLOCK_K = 64`.
The wide `SPLIT_K` range matters because the optimal split is **non-monotonic in
the shape** - an A100 sweep found attn-qkv preferring 8, llama-ffn 4, and qwen 8 -
so a fixed heuristic regresses some shapes while the autotuner picks the winner.
`reset_to_zero=["C"]` is mandatory here: the kernel accumulates into `C` with
`atomic_add`, so the autotuner must re-zero `C` before each timing trial or the
partial sums corrupt both the measurement and the final result.
